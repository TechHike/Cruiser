...
function Start-Cruiser {
	param([switch]$Server, [switch]$Setup, [switch]$Background)

	if ($Setup) {
		(New-Object Net.WebClient).DownloadString("http://psget.net/GetPsGet.ps1") | Invoke-Expression
		Install-Module PSCX
		
		Write-Host "Cruiser setup complete." -ForegroundColor Green
	...
	$script:global = @{
		...
		"status_xslt_path" = ("{0}\Cruiser-Status.xslt" -f $global:CruiserInstallDirectory);
	}
	
	if ($Background) {
		Stop-Cruiser
		Start-Job -Name "Cruiser" -ScriptBlock { 
			param([string]$ModuleDir, [bool]$Server, [string]$PWD)
			#Start-Cruiser -Server $Server 
			#$pwd
			. "$ModuleDir\Cruiser.ps1"
			Set-Location $PWD
			Start-Cruiser -Server $Server 
		} -ArgumentList $global:CruiserInstallDirectory, $Server, $PWD
	} else {
...
function Save-History {
	...
	$history.updated = (Get-Date)
	$history.updated_str = ("{0:yyyy-MM-dd h:mm tt}" -f $history.updated)
...
function Exec {
	param([string]$Path, [string]$LogCategory)
	
	if (-not $LogCategory) {
		$LogCategory = "Exec"
	}
	Log $LogCategory $Path
	$ret = Invoke-Expression -Command $Path
	$ret | foreach { Log $LogCategory $_ }
	return $ret
}
